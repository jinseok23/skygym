...
<div class="carousel-item active" style="height:50%; background-image: url('<%=request.getContextPath()%>/img/recommendImg.jpg')">
  ...
  {		%>
  <p style='font-size:24px; font-weight: 800;'><%=memberLoggedIn.getUserName()%><p style='font-size: 17px;'>님의  정보를 바탕으로 체육관을 매칭한 결과입니다.</p>
  <% }
  else
  { %>
  	<p style='font-size: 17px;'> 회원가입을 하시면 회원님께 꼭 맞는 gym을 매칭해드립니다.</p>
<% }  %>  
...
<div class="container">
  	...
  	<article class="box box_space _space" >
		...
		<a href="<%=request.getContextPath() %>/detail/detailView?GYMNumber=<%= g.getGYMNumber() %>" class="_innerLink nclk" style="text-decoration: none; color:#0d2240;">
			<div class="img_box">
						<span style="background-image: url(<%=request.getContextPath()%><%=g.getMainImage() %>);" class="img lazy" title="세부정보 보기">
				</div>
				<div class="info_area">

...
<div class="container">
	...
	<div id="search-container">
		...
		<div id='member-all'>
			<form action="<%=request.getContextPath()%>/admin/gymList">
				<button type='submit' class="btn xet_btn sky medium">전체보기</button>
			</form>
		</div>
	</div>
			...
			<tr>
					...
					<td><%= g.getGYMdistrict() %></td>
			  		<td>
						...
						<%String markName = g.getGYMbranchName().replace(searchKeyword,"<mark style='color:white; background:#0d2240; font-weight:800;'>" + searchKeyword + "</mark>"); %>
						<%=markName%>
						<%}
						else {%>
						<%=g.getGYMbranchName()%>
